...
### Example
We can configure a bash startup script using Shadeform's [Create Instance API](/api-reference/endpoint/instancescreate).
...
After we have base64 encoded our script, we can create a new GPU instance with the startup script included.
...
Once the instance is created, we can get the status and IP address of the instance by calling the [Instances API](/api-reference/endpoint/instances).
