...
<Note> "volume_ids" must be an array of size one. This is because we only allow attaching one volume to one instance right now. Attaching multiple volumes to a single instance is a feature we may add in the future. </Note>
...
This command will display all block devices attached to the instance. You should see your volume listed as a new device, such as `/dev/sdb` or similar.

---

### Step 5: Deleting the Instance and Volume
