...
Shadeform supports a [launch configuration](/getting-started/concepts#launch-configuration) in the [Create Instance API](/api-reference/endpoint/instancescreate) that allows you to provide a docker container image and properties.
...
To launch a VM instance with the vLLM container automatically started, we need to call the [Create Instance API](/api-reference/endpoint/instancescreate) with the `launch_configuration` property.
