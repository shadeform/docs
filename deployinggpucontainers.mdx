---
title: 'Deploying GPU Containers'
---
### Background
Accessing affordable GPU compute often gets you out of flow. Big clouds have complicated UI's and high prices, but 3rd party clouds have no supply guarantees.
Now with the Shadeform API, you can spin up GPU containers on the most affordable compute without leaving your IDE.
...
cd examples/
```
Then in `deploy_container.ipynb` input your Shadeform API Key from [here](https://platform.shadeform.ai/settings/api), and a  `huggingface_token` if the model you need is gated (like Llama2 or Gemma).
...
The notebook leverages a lot of the principles found in [this post](https://docs.shadeform.ai/findingmostaffordablegpus) for using the platform to _find_ affordable GPU's.
